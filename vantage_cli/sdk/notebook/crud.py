...
class NotebookSDK:
    ...
    def _validate_create_params(
        self, server_name: Optional[str], server_options: Optional[Dict[str, Any]]
    ) -> str:
        """Validate required parameters for notebook creation and return partition name."""
        if not server_name:
            raise Abort(
    ...
    def _build_create_input(
        ...
        cluster_name: str,
        partition_name: str,
        server_options: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        """Build the input payload for the createJupyterServer mutation."""
    ...
    async def _handle_transport_error(
        ...
        """Handle GraphQL transport errors by falling back to REST API."""
        logger.warning(
        ...
        try:
            ...
            logger.error("JupyterHub REST fallback failed: %s", rest_exc)
            raise

        fallback_message = (
            rest_response.get("message", "Notebook server creation requested")
    ...
    async def _handle_existing_notebook(
        self, ctx: typer.Context, server_name: str, username: str
    ) -> Dict[str, Any]:
        """Handle case where notebook already exists."""
        logger.info(
    ...
    async def create_notebook(
        ...
        partition_name = self._validate_create_params(server_name, server_options)
        # After validation, server_name is guaranteed to be a string
        assert server_name is not None

        input_payload = self._build_create_input(
            server_name, cluster_name, partition_name, server_options
        )
        ...
        try:
            ...
            if any(indicator in error_message.lower() for indicator in transport_indicators):
                return await self._handle_transport_error(
                    ctx, cluster_name, username, server_name, partition_name, server_options
                )
        ...
        if typename == "NotebookServerAlreadyExists":
            return await self._handle_existing_notebook(ctx, server_name, username)
