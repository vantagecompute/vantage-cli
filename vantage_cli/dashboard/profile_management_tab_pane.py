...
from textual.widgets import Static, DataTable, Button, TabPane, Input, Label
...
class ProfileManagementTabPane(TabPane):
        ...
        with Vertical():
            ...
            with Vertical(id="profile-creation-section"):
                ...
                with Horizontal(id="profile-creation-form"):
                    ...
                    yield Label("URL:")
                    yield Input(placeholder="Vantage URL", id="profile-url-input", value="https://api.vantage.com")
    ...
    def setup_profiles_table(self) -> None:
        ...
        table.add_columns(
            "Name", "URL", "Active", "API Key Status"
        )
    ...
    async def refresh_profiles(self) -> None:
        ...
        try:
            ...
            self.active_profile = next(
                (profile for profile in profiles_data if profile.is_active),
                None
            )
    ...
    def update_status_display(self) -> None:
        ...
        status_widget.update(f"Status: Ready ({profile_count} profiles, active: {active_profile_name})")
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if event.data_table.id == "profiles-table":
            ...
            selected = next(
                (profile for profile in self.profiles if profile.name == profile_name),
                None
            )
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "refresh-profiles-btn":
            ...
            self.notify(f"Editing profile {self.selected_profile.name} - Not implemented yet", severity="warning")
