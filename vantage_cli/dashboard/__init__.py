...
from textual.widgets import (
    Header, Static, ProgressBar, DataTable, 
    RichLog, TabbedContent, TabPane, Button
)
...
class DashboardApp(App):
    ...
    def _get_default_services(self) -> List[ServiceConfig]:
        ...
        return [
            ...
            ServiceConfig("slinky", "https://slinky.api.local", "🔗", ["cert-manager", "prometheus"]),
        ...
        with Horizontal(id="tab-and-buttons-container"):
                    ...
                    with Vertical():
                        with Horizontal(id="main-panel"):
                            ...
                            with Vertical(id="right-content"):
                                ...
                                yield Static(f"🚀 {self.platform_info['name']}", classes="section-header")
                                with Vertical(id="vantage-platform"):
                                    yield Static("[bold]🔗 Access your cluster[/bold]")
                                    yield Static(f"    {self.platform_info['cluster_url']}", classes="url-link")
                                    yield Static("")
                                    yield Static("[bold]📓 Create a notebook[/bold]")
                                    yield Static(f"    {self.platform_info['notebooks_url']}", classes="url-link")
                                    yield Static("")
                                    yield Static("[bold]📚 Documentation[/bold]")
                                    yield Static(f"    {self.platform_info['docs_url']}", classes="url-link")
                                    yield Static("")
                                    yield Static("[bold]💬 Support[/bold]")
                                    yield Static(f"    {self.platform_info['support_email']}", classes="url-link")
            ...
            if self.config.enable_controls:
                with Vertical(id="control-buttons"):
                    ...
                    yield Button("Restart", id="restart-btn", variant="primary", classes="tab-button")
        
        # Footer
        yield Static(f"🚀 {self.platform_info['name']} - {self.config.title} | Press Ctrl+C to quit", id="footer")
    ...
    def refresh_stats(self):
        ...
        try:
            ...
            self.query_one("#memory-stat", Static).update(f"Memory: {psutil.virtual_memory().percent:.1f}%")
