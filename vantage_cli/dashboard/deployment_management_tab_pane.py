...
from textual.widgets import Static, DataTable, Button, TabPane, Select
...
class DeploymentManagementTabPane(TabPane):
        ...
        with Vertical():
            ...
            with Horizontal(id="deployment-filters"):
                yield Select([
                    ...
                    ("GCP", "gcp"),
                    ("Azure", "azure")
                ], prompt="Cloud:", value="all", id="cloud-filter-select")
                yield Select([
                    ("All Status", "all"),
                    ("Active", "active"),
                    ("Inactive", "inactive"),
                    ("Error", "error")
                ], prompt="Status:", value="all", id="status-filter-select")
            ...
            with Horizontal(id="deployment-actions"):
                ...
                yield Button("🔄 Refresh Status", id="refresh-deployment-status-btn", disabled=True)
                yield Button("⚙️ Manage", id="manage-deployment-btn", disabled=True)
                yield Button("🗑️ Delete", id="delete-deployment-btn", disabled=True, variant="error")
    ...
    def setup_deployments_table(self) -> None:
        ...
        table.add_columns(
            "Name", "App", "Cluster", "Cloud", "Status", "Created", "Active"
        )
    ...
    def update_deployments_table(self) -> None:
        ...
        for i, deployment in enumerate(self.deployments):
            ...
            logger.debug(f"Adding row: {name}, {app_name}, {cluster_name}, {cloud}, {status}, {created}, {is_active}")
            table.add_row(name, app_name, cluster_name, cloud, status, created, is_active, key=deployment.deployment_id)
    ...
    def update_status_display(self) -> None:
        ...
        status_widget.update(f"Status: Ready ({deployment_count} deployments, {active_count} active)")
    ...
    def _display_deployment_object(self, deployment: Deployment) -> None:
        ...
        display_items = [
            ...
            ("Compatible Integrations", ", ".join(deployment.compatible_integrations) if deployment.compatible_integrations else "None"),
        ]
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        logger.debug(f"Row selected event: table_id={event.data_table.id}, row_key={event.row_key}")
        ...
        if event.data_table.id == "deployments-table":
            ...
            selected = next(
                (deployment for deployment in self.deployments if deployment.deployment_id == deployment_id),
                None
            )
    ...
    async def delete_deployment(self) -> None:
        ...
        try:
            ...
            if success:
                ...
                self.deployments = [d for d in self.deployments if d.deployment_id != deployment_id]
