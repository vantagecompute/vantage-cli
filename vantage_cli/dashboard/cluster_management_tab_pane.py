...
from textual.widgets import Static, DataTable, Button, TabPane
...
from vantage_cli.sdk.cluster import list as list_clusters, get as get_cluster
...
class ClusterManagementTabPane(TabPane):
    ...
    def setup_clusters_table(self) -> None:
        ...
        table.add_columns(
            "Name", "Status", "Provider", "Owner", "Description"
        )
        table.cursor_type = "row"
        table.show_cursor = True
        logger.debug(f"Clusters table setup complete. cursor_type={table.cursor_type}, show_cursor={table.show_cursor}")
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if event.data_table.id == "clusters-table":
            ...
            selected = next(
                (cluster for cluster in self.clusters if cluster.get("name") == cluster_name),
                None
            )
