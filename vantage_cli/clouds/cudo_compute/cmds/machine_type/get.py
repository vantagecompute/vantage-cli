...
async def get_machine_type(
    ...
    Retrieves detailed information about a machine type including pricing,
    ...
    with RenderStepOutput.json_bypass(formatter):
        try:
            machine_type_details = await sdk.get_vm_machine_type(
            ...
            RenderStepOutput.render_output(
                ...
                formatter=formatter,
                title=f"Machine Type: {machine_type}",
            )
        except Exception as e:
            error_msg = str(e)
            if "404" in error_msg or "not found" in error_msg.lower():
                typer.echo(
                    f"Machine type '{machine_type}' not found in data center '{datacenter_id}'",
                    err=True,
                )
            else:
